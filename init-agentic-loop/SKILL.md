...
Read and follow `../agentic-dev-loop/references/init-project.md` in full — repo inspection, the interview (including the UI-motion architecture question — Spline 3D vs scroll animation — whenever the project has marketing/landing UI in scope), then synthesizing ROADMAP.md, TODO.md, ARCHITECTURE.md, API.md, CHANGELOG.md, EXECUTION_LOG.md, and KNOWN_ISSUES.md from `../agentic-dev-loop/assets/doc-templates/`.
...
Stop after presenting the summary per init-project.md §4 — don't continue into the main loop's task execution unless the user says to (this command is for initializing/re-initializing on demand, not for running the loop itself).
